...
export const combineNumbers = (Board, direction, Score = 0, setScore = () => {}, setGameOver = () => {}) => {
        ...
        if(direction === "left") {
            for(let col = 0; col < 3; col++){
                if(newBoard[row][col] > 0 && newBoard[row][col] === newBoard[row][col + 1]){
                    ...
                    setScore(newScore)
                    if(newBoard[row][col] === 11) setGameOver({isGameOver: true, message: "You won!"})
                }
            }
        } else {
...
export const rotateAndCombine = (Board, direction, Score = 0, setScore = () => {}, setGameOver = () => {}) => {
    let newBoard = rotateBoard(Board, "left")
    newBoard = combineNumbers(newBoard, direction === "up" ? "left" : "right", Score, setScore, setGameOver)
...
const handleKeyDown = (e, Board, setBoard, Score, setScore, setGameOver) => {
    ...
    switch(e.keyCode){
        ...
        case 87:
            direction = "up"
            newBoard = rotateAndCombine(Board, direction, Score, setScore, setGameOver)
            break
        case 40:
        case 83:
            direction = "down"
